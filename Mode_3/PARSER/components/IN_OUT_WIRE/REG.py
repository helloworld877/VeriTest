...
class REG(node):
    def __init__(self, Type, size, name, endian = "little"):
        super().__init__(Type)
        self.name = name
    ...
    def pass_output_to_ports(self, output, connection):
        ...
        if isinstance(connection.destination, OUTPUT) or connection.destination.Type == "WIRE" or isinstance(connection.destination, REG):
